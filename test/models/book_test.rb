...
class BookTest < ActiveSupport::TestCase
  ...
  test "入金は正常であること" do
    ...
    assert_equal @book_depo.updated_balance, 20000
  end

  test "（口座残高がない状態からの）出金はエラーになること" do
    assert_not @book_pay.valid?
    assert_equal @book_pay.updated_balance, -5000
  end

  test "入金したあとの出金は正常になること" do
    @book_depo.save
    assert @book_pay.valid?
    assert_equal @book_pay.updated_balance, 20000 - 5000
  end
  ...
  test "入金金額以上の出金はエラーになること" do
    ...
    assert_not @book_pay.valid?
    assert_equal @book_pay.updated_balance, 20000 - 20010
  end
  ...
  test "入金・出金レコード保存後、出金側の金額を残高がマイナスになるように大きく更新できないこと" do
    ...
    assert_not @book_pay.valid?
    assert_equal @book_pay.updated_balance, 20000 - 30000
  end
  ...
  test "入金・出金レコード保存後、入金側の金額を残高がマイナスになるように小さく変更できないこと" do
    ...
    assert_equal @book_depo.updated_balance, 4000 - 5000
  end
